...
`admin.html`'s "Site Card Preview" panel (`renderSitePreview`) re-implements `app.js`'s card markup so the admin can see the real card before publishing. It reuses `style.css`'s actual `.card` rules unmodified by re-scoping the same CSS custom property names (`--bg`, `--surface`, `--accent`, etc.) inside `#site-preview`, since the admin UI's own dark theme uses different values for those variables. If you change card markup/structure in `app.js`, mirror the change in `renderSitePreview` (and vice versa) or the preview will lie.
