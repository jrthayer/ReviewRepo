...
**Unlike blocks/tabs, there is no `push*` wrapper for these two files.** Every tag/category edit (add a category, rename a category, add/recategorize a tag, drag a chip onto a category line) only mutates the in-memory `tagCategories`/`tagRegistry` arrays and calls `markTagsDirty()` — no GitHub call per action, since a reorganizing session can easily touch many tags in a row. `tagsDirty` shows a "● Unsaved tag changes" indicator next to the Save button (`#tags-dirty-indicator`); `saveReview()` only pushes `tag-categories.json`/`tags.json` when it's true, bundled with the review into one `ghCommitFiles` commit (see above), then `clearTagsDirty()`. If you have pending tag edits and never click Save, they're lost — same as any other unsaved form field. Any new tag/category mutation should follow this local-only + `markTagsDirty()` shape, not add its own `ghPutJson` call.
