...
There's no dedicated "Manage Tags" UI — categories are managed directly in the tag chip editor (`renderTags()`), which always shows one line per known category (plus a permanent "Uncategorized" line) so every category is a visible drop target even with no tags on the current review yet. "+ Add Category" next to the Tags label, and renaming a category by clicking its label, both use the same trigger/editor-swap shape as "+ Save Block" (`start*`/`confirm*`/`cancel*`, an input plus inline ✓/✗ buttons) — rename just pre-fills the input with the current name (`startRenameTagCategory`/`confirmRenameTagCategory`/`cancelRenameTagCategory`, tracked via `renamingTagCategoryId` since, unlike "+ Add Category", more than one category's editor can potentially be triggered across re-renders). Renaming cascades to every registry tag currently filed under the old name (`tagRegistry.forEach(t => t.category === oldName ? ... )`), since a tag's category is a stored name string, not an id; skipping that cascade would silently orphan those tags back to uncategorized. There's deliberately no delete-category or delete-tag UI — renaming covers fixing a mistake, and an unused registry tag is harmless.
